...
int main (int argc, const char * argv[]){
	...
	try{
		...
		if (opType == "-add") { //add soundFile1 and soundFile2
			...
			if(numberOfChannels==1){	//mono
				if (bitCount == 8){
					BTHJAC013::Audio<int8_t,1> sample1 (infile1, sampleRate);//Create audio Sample #1,8bit
					BTHJAC013::Audio<int8_t,1> sample2 (infile2, sampleRate);//Create audio Sample #2,8bit
					...
					BTHJAC013::Audio<int16_t,1> sample2 (infile2, sampleRate);//Create audio Sample #2,16bit
					(sample1 + sample2).writeToFile(outFile);			//Save audio (write to file)
				...
				if (bitCount == 8){
					...
					BTHJAC013::Audio<int8_t,2> sample2 (infile2, sampleRate);//Create audio Sample #2,8bit
					(sample1 + sample2).writeToFile(outFile);			//Save audio (write to file)
				} else if (bitCount == 16){
					BTHJAC013::Audio<int16_t,2> sample1 (infile1, sampleRate);//Create audio Sample #1,16bit
					BTHJAC013::Audio<int16_t,2> sample2 (infile2, sampleRate);//Create audio Sample #2,16bit
					(sample1 + sample2).writeToFile(outFile);			//Save audio (write to file)
				} else {				//some extra resundency
					std::cout << "Don't know how this slipped through, but only 8 and 16 bit counts are currrently supported." << std::endl;
				}
			}				
		} else if (opType == "-cut") { //remove samples over range [r1,r2] (inclusive) (assumesone sound file)
			...
			if(numberOfChannels==1){	//mono
				if (bitCount == 8){
					BTHJAC013::Audio<int8_t,1> sample1 (infile, sampleRate);//Create audio Sample #1,8bit
					(sample1^excludeSegment).writeToFile(outFile);			//Save audio (write to file)
				} else if (bitCount == 16){
					BTHJAC013::Audio<int16_t,1> sample1 (infile, sampleRate);//Create audio Sample #1,16bit
					(sample1^excludeSegment).writeToFile(outFile);			//Save audio (write to file)
				} else {				//some extra resundency
					std::cout << "Don't know how this slipped through, but only 8 and 16 bit counts are currrently supported." << std::endl;
				}
				...
				if (bitCount == 8){
					BTHJAC013::Audio<int8_t,2> sample1 (infile, sampleRate);//Create audio Sample #1,8bit
					(sample1^excludeSegment).writeToFile(outFile);			//Save audio (write to file)
				} else if (bitCount == 16){
					BTHJAC013::Audio<int16_t,2> sample1 (infile, sampleRate);//Create audio Sample #1,16bit
					(sample1^excludeSegment).writeToFile(outFile);			//Save audio (write to file)
				} else {				//some extra resundency
					std::cout << "Don't know how this slipped through, but only 8 and 16 bit counts are currrently supported." << std::endl;
				}
			...
			if(numberOfChannels==1){	//mono
				if (bitCount == 8){
					BTHJAC013::Audio<int8_t,1> sample1 (infile1, sampleRate);//Create audio Sample #1,8bit
					BTHJAC013::Audio<int8_t,1> sample2 (infile2, sampleRate);//Create audio Sample #2,8bit
					(sample1.selectRange(range1 * sampleRate, range2 * sampleRate) + sample2.selectRange(subRange1, subRange2)).writeToFile(outFile);
				} else if (bitCount == 16){
					BTHJAC013::Audio<int16_t,1> sample1 (infile1, sampleRate);//Create audio Sample #1,16bit
					BTHJAC013::Audio<int16_t,1> sample2 (infile2, sampleRate);//Create audio Sample #2,16bit
					(sample1.selectRange(range1 * sampleRate, range2 * sampleRate) + sample2.selectRange(subRange1, subRange2)).writeToFile(outFile);
			...
			if(numberOfChannels==1){	//mono
				if (bitCount == 8){
					...
					BTHJAC013::Audio<int16_t,1> sample1 (infile1, sampleRate);//Create audio Sample #1,8bit
					BTHJAC013::Audio<int16_t,1> sample2 (infile2, sampleRate);//Create audio Sample #2,8bit
					(sample1 | sample2).writeToFile(outFile);			//Save audio (write to file)
				} else {				//some extra resundency
					std::cout << "Don't know how this slipped through, but only 8 and 16 bit counts are currrently supported." << std::endl;
				}
				...
				if (bitCount == 8){
					...
					BTHJAC013::Audio<int16_t,2> sample1 (infile1, sampleRate);//Create audio Sample #1,8bit
					BTHJAC013::Audio<int16_t,2> sample2 (infile2, sampleRate);//Create audio Sample #2,8bit
					(sample1 | sample2).writeToFile(outFile);			//Save audio (write to file)
				} else {				//some extra resundency
					std::cout << "Don't know how this slipped through, but only 8 and 16 bit counts are currrently supported." << std::endl;
				}
			...
			if(numberOfChannels==1){	//mono
				if (bitCount == 8){
					BTHJAC013::Audio<int8_t,1> sample1 (infile, sampleRate);//Create audio Sample #1,8bit
					std::cout << "RMS : " << infile << " = " << sample1.calcRMS() << std::endl;
				} else if (bitCount == 16){
					BTHJAC013::Audio<int16_t,1> sample1 (infile, sampleRate);//Create audio Sample #1,16bit
					std::cout << "RMS : " << infile << " = " << sample1.calcRMS() << std::endl;
				} else {				//some extra resundency
				...
				if (bitCount == 8){
					BTHJAC013::Audio<int8_t,2> sample1 (infile, sampleRate);//Create audio Sample #1,8bit
					std::cout << "Left RMS : " << infile << " = " << sample1.calcRMS().first << std::endl;
					std::cout << "Right RMS : " << infile << " = " << sample1.calcRMS().second << std::endl;
				} else if (bitCount == 16){
					BTHJAC013::Audio<int16_t,2> sample1 (infile, sampleRate);//Create audio Sample #1,16bit
					std::cout << "Left RMS : " << infile << " = " << sample1.calcRMS().first << std::endl;
					std::cout << "Right RMS : " << infile << " = " << sample1.calcRMS().second << std::endl;
